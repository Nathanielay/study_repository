...
### 4.7 场景短文学习（新增）
- 按场景生成 150-2000 个英文单词短文，场景包含：需求评审、调试、代码评审、上线事故、写文档。
- 词来源为收藏词库，单篇覆盖约 35 个词（最大 50），可按主题小幅浮动。
- 输出结构包含：英文短文、中文意译、中文逐句语法解析。
- 英文与中文翻译中需高亮覆盖词。
- 支持手动指定场景词参与生成。
- 生成成功后持久化存储到 MySQL，并可在短文列表中查看历史文章。

### 4.8 默写与评分（新增）
- 整篇默写，允许先查看中文意译与语法解析再开始。
- 判分规则：完全一致，忽略标点与大小写。
- 结果展示英文原文高亮：拼写错误（黄色）、缺漏（蓝色）、多余（橙色，附在原文后）。
- 提交后调用 LLM 分析语法、拼写与易漏点。
- 错词计入错题本。

### 4.9 复盘词网（新增）
- 每次复盘 5 个核心词，每个词 2 个固定短语。
- 50% 新词，来源优先级：本篇 + 错题本优先 → 同场景词库 → 词根/词缀 → 同义/反义。
- 允许跨场景连接，展示中文释义。
- 词网支持跳转词详情。
...
- 单词书列表：所有书与单词数。
- 每本书学习进度：当前索引。
- 场景短文：短文内容、翻译、语法解析、关联词列表。
- 场景短文列表：历史生成短文索引与摘要信息。
- 默写记录：用户输入、评分结果、错词列表。
- 错题本：错词统计与最近错误时间。
- 复盘词网：核心词、短语、新词标记与来源。
- 异步任务：生成任务状态、失败原因、关联短文。
...
- 运行依赖 MySQL 连接可用。
- 建议使用 Docker 蓝绿发布（3001/3002）+ Nginx 切流量，部署前进行健康检查（/api/health）。
